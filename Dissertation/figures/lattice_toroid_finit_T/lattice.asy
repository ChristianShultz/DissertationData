...
real ddiff = diff - 0.005;
real bdiff = diff + 0.005;
pair l1 = (midpoint + ddiff,1/9);
pair l2 = (midpoint - ddiff,1/9);
pair l3 = (midpoint - bdiff,1/9);
pair l4 = (midpoint + bdiff,1/9);
pair l5 = (midpoint +1.3*bdiff,1/9);

// Torus.drawSurfacePath(makeline(l2, l1), p=linewidth);
Torus.drawSurfacePath(makeline(l3, l4,ucycles=-1), p=linewidth+blue);

drawLayeredPaths();    // Draw the paths (with hidden path removal) that were computed in the previous code block.

path3 PS = Torus.onSurface(makeline(l2, l1));
path3 PL = Torus.onSurface(makeline(l5, l4));

draw(project(PS), arrow=ArcArrow(),p=linewidth+red);
draw(project(PL), arrow=ArcArrow(),p=linewidth+blue);
