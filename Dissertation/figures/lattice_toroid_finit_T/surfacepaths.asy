...
struct SmoothSurface {
    ...
    void drawMesh(int nu, int nv, int npath=10, pen meshpen=gray(0.6)) {
        for (real u = 0; u < 1; u += 1/nu)
            drawSurfacePath(graph(new pair(real v) {return (u,v);}, 0, 1, n=npath), p=meshpen, onTop=false);
        for (real v = 0; v < 1; v += 1/nv)
            drawSurfacePath(graph(new pair(real u) {return (u,v);}, 0, 1, n=npath), p=meshpen, onTop=false);
    }
    ...
    /*
       ...
       void shade(material surfacepen=material(gray, ambientpen=white),
       light light=currentlight, int nu=20, int nv=nu)
       {

       typedef penAtPoint[] Layer;
       Layer[] layers;
       ...
       for (int j = 0; j < nv; ++j) {
       real v = j/nv;
       int layer = numSheetsHiding(u,v);
       penAtPoint toAdd;
       toAdd.thepen = color(u, v, surfacepen, light);
       toAdd.thepoint = this.project(point(u,v));
       layers[layer].push(toAdd);
       }
       }
